...
/*!
    ...
    @return Adafruit_SSD1306_mod object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_mod::Adafruit_SSD1306_mod(uint8_t w, uint8_t h, TwoWire *twi,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_mod::Adafruit_SSD1306_mod(uint8_t w, uint8_t h,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_mod::Adafruit_SSD1306_mod(uint8_t w, uint8_t h, SPIClass *spi,
...
/*!
    @brief  DEPRECATED constructor for SPI SSD1306 displays, using software
            ...
            one of the SSD1306_* size defines in Adafruit_SSD1306_mod.h. New
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_mod::Adafruit_SSD1306_mod(int8_t mosi_pin, int8_t sclk_pin,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_mod::Adafruit_SSD1306_mod(int8_t dc_pin, int8_t rst_pin,
...
/*!
    @brief  DEPRECATED constructor for I2C SSD1306 displays. Provided for
            ...
            defines in Adafruit_SSD1306_mod.h. New code should NOT use this.
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_SSD1306_mod::Adafruit_SSD1306_mod(int8_t rst_pin) :
...
/*!
    @brief  Destructor for Adafruit_SSD1306_mod object.
*/
Adafruit_SSD1306_mod::~Adafruit_SSD1306_mod(void) {
  if(buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void Adafruit_SSD1306_mod::SPIwrite(uint8_t d) {
  if(spi) {
    (void)spi->transfer(d);
  } else {
...
void Adafruit_SSD1306_mod::ssd1306_command1(uint8_t c) {
  if(wire) { // I2C
...
void Adafruit_SSD1306_mod::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if(wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint8_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void Adafruit_SSD1306_mod::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
*/
boolean Adafruit_SSD1306_mod::begin(uint8_t vcs, uint8_t addr, boolean reset,
  boolean periphBegin) {

  if((!buffer) && !(buffer = (uint8_t *)malloc((HEIGHT + 7) / 8)))
    return false;
  ...
  static const uint8_t PROGMEM init3[] = {
    ...
    0x01,                                 // 0x01 vertical addressing mode! -BD
...
*/
void Adafruit_SSD1306_mod::clearDisplay(void) {
  memset(buffer, 0, ((HEIGHT + 7) / 8));
}
...
*/
uint8_t *Adafruit_SSD1306_mod::getBuffer(void) {
  return buffer;
}
...
/*!
    @brief  Set up for pushing columns in RAM to SSD1306 display.
    @return None (void).
*/
void Adafruit_SSD1306_mod::startDisplay(void) {
  TRANSACTION_START
...
/*!
    @brief  Push a single column in RAM to SSD1306 display.
    @return None (void).
*/
void Adafruit_SSD1306_mod::displayColumn(void) {
  uint16_t count = ((HEIGHT + 7) / 8);
  uint8_t *ptr   = buffer;
  if(wire) { // I2C
    while(count--) {
      if(bytesOut >= WIRE_MAX) {
...
/*!
    @brief  Finalization for pushing columns in RAM to SSD1306 display.
    @return None (void).
*/
void Adafruit_SSD1306_mod::endDisplay(void) {
  if(wire) { // I2C
    wire->endTransmission();
  } else { // SPI
  }
...
void Adafruit_SSD1306_mod::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void Adafruit_SSD1306_mod::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void Adafruit_SSD1306_mod::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void Adafruit_SSD1306_mod::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void Adafruit_SSD1306_mod::stopscroll(void) {
  TRANSACTION_START
...
*/
void Adafruit_SSD1306_mod::invertDisplay(boolean i) {
  TRANSACTION_START
...
*/
void Adafruit_SSD1306_mod::dim(boolean dim) {
